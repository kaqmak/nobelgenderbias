...
myggsaver <- function(gg, filename, width = 5,height = 3){
  ...
  ggsave(gg,filename = fpath,device = 'pdf',width = width,height = height)
}
```
...
First we load the number of laureates within each field for each year seperated into gender
```{r}
...
Next we extract the total number of Nobel laureates as taken from https://www.nobelprize.org :
```{r}
...
gg <- allNobelDF %>% mutate(nbl_m=nbl-nbl_f) %>% gather(key=key, value=value,nbl_m, nbl_f) %>% 
  ...
  scale_fill_brewer(palette = "Set1",name = 'Nobel laureates', labels=c("Female","Male"))+
  theme_Publication(darkTheme = darkTheme)
